...
export default class TokenPage extends Component<TokenPageProps, State> {
  ...
  async componentDidMount() {
    await this.setState({
      ...this.state,
      tokenAddress: await api.token(
        this.props.info.stakerAddress,
        this.props.provider
      ),
    });
    await this.setState({
      ...this.state,
      tenderTokenAddress: await api.tenderToken(
        this.props.info.stakerAddress,
        this.props.provider
      ),
    });
    //await this.setState({
    //  ...this.state,
    //  sharePrice: await api.sharePrice(
    //    this.props.info.stakerAddress,
    //    this.props.provider
    //  ),
    //});
  ...
  tokenAllowance = async (provider: any) => {
    ...
    await this.setState({
      ...this.state,
      tokenAllowance: await api.allowance(
        address,
        this.props.info.stakerAddress,
  ...
  tenderTokenAllowance = async (provider: any) => {
    ...
    await this.setState({
      ...this.state,
      tenderAllowance: await api.allowance(
        address,
        this.props.info.stakerAddress,
        this.state.tenderTokenAddress,
        this.props.provider
    ...
    if (
      ...
      try {
        await api.approve(
          ...
          this.props.info.stakerAddress,
          this.state.tokenAddress,
          this.props.provider.getSigner()
        );
        await this.setState({
          ...this.state,
          tokenAllowance:
            parseInt(this.state.tokenAllowance, 10) +
            parseInt(this.state.depositAmount, 10).toString(),
        });
      } catch (e) {
        console.log(e);
        return;
    ...
    if (
      ...
      try {
        ...
        this.setState({
          ...
          tenderAllowance:
            parseInt(this.state.tenderAllowance, 10) +
            parseInt(this.state.withdrawAmount, 10).toString(),
        });
      } catch (e) {
        console.log(e);
        return;
  ...
  apy = () => {
    const apy =
      (parseFloat(this.state.currentSharePrice) -
        parseFloat(this.state.startSharePrice)) *
      100;
    return apy;
  };
      ...
      <>
        <Row>
          <Col lg={{ span: 6 }} md={{ span: 6 }}>
            <div className="share numerator">
              <span className="price">{this.state.currentSharePrice}</span>{" "}
              {this.props.info.symbol}
            </div>
            <div className="share denominator">t{this.props.info.symbol}</div>
          </Col>
          <Col lg={{ span: 6 }} md={{ span: 6 }}>
            <div className="share apy">{this.apy()}%</div>
          </Col>
        </Row>
      </>
    );
  ...
  render() {
    ...
    return (
      <>
        <TransactionModal
          isOpen={this.state.transactionModalOpen}
                ...
                {this.state.activeTab === "deposit" && (
                  <>
                    ...
                    <Form onSubmit={this.handleDeposit}>
                      ...
                      <Button
                        disabled={this.state.depositAmount == "0"}
                        style={{ width: "100%" }}
                        type="submit"
                      >
                        {depositText()}
